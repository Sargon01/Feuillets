...
export async function loadNativeReviewLocalState(app: App, location: NativeReviewStorageLocation, reviewId: string): Promise<NativeReviewLocalState> {
  ...
  if (!(file instanceof TFile)) throw new Error("Invalid local-state.json");
  let state: unknown; try { state = JSON.parse(await app.vault.read(file)); } catch { throw new Error("Corrupted local-state.json"); }
  if (!state || typeof state !== "object" || (state as { version?: unknown }).version !== 1 || ((state as { archivedAt?: unknown }).archivedAt !== undefined && typeof (state as { archivedAt?: unknown }).archivedAt !== "string")) throw new Error("Invalid local-state.json");
...
export async function setNativeReviewArchived(app: App, location: NativeReviewStorageLocation, reviewId: string, archived: boolean): Promise<NativeReviewLocalState> {
  ...
  if (file instanceof TFile) await app.vault.modify(file, text); else if (!file) await app.vault.create(path, text); else throw new Error("Invalid local-state.json");
  return state;
}
