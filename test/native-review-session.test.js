...
function session(reviewId = "review-1", localRole = "author") {
  return {
    ...
    rounds: [{ round: 1, createdAt: at, sent: pkg("package-1") }],
  };
}
...
test("tours continus, append et currentReviewRound", () => {
  const value = session(); assert.equal(currentReviewRound(value).round, 1);
  const second = appendReviewRound(value, "received", pkg("package-2", "2026-08-13T11:00:00.000Z"));
  assert.equal(second.round, 2); assert.equal(currentReviewRound(value), second);
  const gap = session(); gap.rounds.push({ round: 3, createdAt: at, sent: pkg("package-3") }); assert.throws(() => validateReviewSession(gap), InvalidReviewSessionError);
  const duplicate = session(); duplicate.rounds.push({ round: 1, createdAt: at, sent: pkg("package-3") }); assert.throws(() => validateReviewSession(duplicate), InvalidReviewSessionError);
});

test("enregistre sent et received, idempotence et protections de package", () => {
  const value = session();
  recordReviewRoundPackage(value, 1, "received", pkg("package-2"));
  assert.equal(value.rounds[0].received.packageId, "package-2");
  assert.equal(recordReviewRoundPackage(value, 1, "received", pkg("package-2")), value.rounds[0]);
  assert.throws(() => recordReviewRoundPackage(value, 1, "received", pkg("replacement")), InvalidReviewSessionError);
  appendReviewRound(value, "sent", pkg("package-3"));
  assert.throws(() => recordReviewRoundPackage(value, 2, "received", pkg("package-2")), InvalidReviewSessionError);
});
...
test("sauvegarde et recharge sans toucher aux autres fichiers", async () => {
  ...
  appendReviewRound(value, "received", pkg("package-2")); value.updatedAt = "2026-08-13T11:00:00.000Z";
