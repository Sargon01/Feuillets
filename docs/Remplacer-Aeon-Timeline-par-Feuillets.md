...
Dans Feuillets, ces éléments se trouvent dans le panneau **Recherche**.
...
Feuillets propose également ce contrôle dans le panneau **Contexte**.
...
Le panneau Contexte peut ensuite afficher la fiche du lieu lorsque son nom ou son alias apparaît dans le passage.
...
- la recherche du panneau Recherche ;
- les filtres par tag ;
...
Feuillets reprend une partie de cette logique directement dans le panneau **Contexte**.
...
→ écrire la scène
→ consulter les alertes du panneau Contexte
```
...
Les incohérences apparaissent dans le panneau Contexte au moment où le personnage, l’objet ou l’événement est cité.
