...
- créer une note de travail dans l’onglet Notes.
...
Dans Feuillets, ces informations sont réparties entre plusieurs zones de l’Inspecteur et vues :
...
- les notes de travail de l’onglet Notes ;
...
Feuillets ajoute une fonction plus contextuelle : le section **Contexte** de l’onglet **Notes**.
...
Dans Feuillets, le section **Plan du feuillet** de l’onglet **Notes** affiche les titres Markdown du document :
...
Dans Obsidian, vous pouvez utiliser un module spécialisé. **Feuillets-Grammalecte** peut alimenter l’onglet Relecture via l’API de Feuillets ; un autre plugin Obsidian peut également être utilisé selon votre langue et vos préférences.

### Différence

Ulysses offre une solution plus intégrée.

Feuillets sépare l’atelier d’écriture du moteur de correction. Le noyau n’embarque donc aucun moteur grammatical ; la correction linguistique est facultative et fournie par un compagnon ou un autre plugin.
...
Feuillets propose plus d’onglets, de vues et de concepts. Ils peuvent être masqués, mais l’environnement reste plus vaste.

## Le Dashboard centralisé

Les informations sont réparties entre Notes — qui inclut les propriétés et le Contexte —, Recherche, Analyse/Statistiques et Journal.
